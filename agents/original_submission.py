...
_W_SCORE = 307.227711714
_W_THREAT = 460.841082412
_W_HMAP = 0.3
_W_ODD_EVEN = 80
_HEATMAP = np.array([
    [3, 4, 5, 7, 5, 4, 3],
...
_TD_WEIGHTS = [0.870818391371, -1.80722183361, 2.71082989654, 1.61992222556, -2.04895128865, 0.720797540315, -0.716279778575, 0.743296680551, 0.288228866283, -0.933056547131, -0.287566020457]
# === BUILD_CONFIG_END ===
...
def score_teacher_moves(board, mark, depth=5, teacher_depth=None):
    """
    Stable teacher-scoring API for azlite.teacher_data.

    Returns move scores from the current player's perspective without relying on
    the workspace root submission.py, which may be overwritten by azlite export.
    """
    global _nodes, _search_start, _timed_out, _last_search_mode
    global _search_soft_budget_ms, _search_hard_budget_ms

    board_arr = np.asarray(board, dtype=np.int8).reshape(ROWS, COLS)
    board_list = board_arr.reshape(-1).tolist()
    current_mark = 1 if int(mark) == 1 else 2
    search_depth = max(1, int(teacher_depth if teacher_depth is not None else depth))

    scores = np.full(COLS, -math.inf, dtype=np.float64)
    valid = [c for c in range(COLS) if board_list[c] == 0]
    if not valid:
        return scores, 0.0
    ...
    # Teacher labeling is offline work, so keep the hard budget generous to
    # prevent accidental timeout-driven fallback in worker processes.
    _nodes = 0
    _search_start = time.perf_counter()
    _timed_out = False
    _last_search_mode = f"teacher_d{search_depth}"
    _search_soft_budget_ms = 60000.0
    _search_hard_budget_ms = 60000.0

    ordered = _order_moves(valid, -1, search_depth, b_self, b_opp, heights, 1.0)
    for col in ordered:
        pos = _drop_pos(heights, col)
        mirror_pos = _MIRROR_POS[pos]
        mask = np.uint64(1) << np.uint64(pos)
        new_self = b_self | mask

        if _has_won(new_self):
            scores[col] = MATE_SCORE
            continue

        heights[col] += 1
        child_hash = _zobrist_update(hash_key, pos, 1)
        child_mirror_hash = _zobrist_update(mirror_hash_key, mirror_pos, 1)
        child_score, _ = _negascout(
            new_self,
            b_opp,
            heights,
        ...
        if _timed_out:
            # Keep teacher generation stable even if deep search overruns.
            _timed_out = False
            heights[col] += 1
            scores[col] = float(evaluate(new_self, b_opp, heights))
            heights[col] -= 1
        else:
            scores[col] = -float(child_score)

    legal_scores = scores[np.isfinite(scores)]
    if legal_scores.size == 0:
        return scores, 0.0

    best_score = float(np.max(legal_scores))
    value = float(math.tanh(best_score / MATE_SCORE))
